...
function [badTrials,allBadTrials,badTrialsUnique,badElecs,totalTrials,slopeValsVsFreq] = findBadTrialsWithEEG_newSRC(subjectName,expDate,protocolName,folderSourceString,gridType,badEEGElectrodes,nonEEGElectrodes,impedanceTag,capType,saveDataFlag,badTrialNameStr,displayResultsFlag)
...
if exist('FsEye','var') && ~isempty(FsEye)
    badEyeTrials = findBadTrialsFromEyeData_newSRC(eyeDataDeg,eyeRangeMS,FsEye,checkPeriod)'; % added by MD 10-09-2017; Modified by MD 03-09-2019
else
    badEyeTrials = [];
end
...
for iElec=1:numElectrodes
    ...
    for i = 1: size(eegData,2)
        ...
        analogData = squeeze(eegData(iElec,i,:,:));
        ...
        allBadTrials{iElec} = unique(cat(2,allBadTrials{iElec},allBadTrialsTMP));
        
        % Calculate number of unique bad trials for each thresholding criterion
        timeThresTMP = originalTrialInds(badTrialsTimeThres);
        freqThresTMP =  originalTrialInds(setdiff(badTrialsFreqThres,badTrialsTimeThres));
        badTrialsUnique.timeThres{iElec} = unique(cat(2,badTrialsUnique.timeThres{iElec},timeThresTMP));
