...
function [handle,Zi,grid,Xi,Yi] = topoplot_murty(Values,loc_file,varargin)
...
%    patch(headx,heady,ones(size(headx)),0.5.*[255 235 211]./255,'linestyle','none','facealpha',0.25); hold on % MD: 11-April-2019
end
...
%%%%%%%% Mark specified electrode locations with coloured disks  %%% MD 04-March-2019
try,
    if strcmpi(STYLE,'blank') && length(COLORARRAY)==length(x)% if mark-selected-channel-locations mode
        hp2 = scatter(y,x,150,COLORARRAY,'filled');%,'markeredgecolor','k');
    end
catch, end;
%
