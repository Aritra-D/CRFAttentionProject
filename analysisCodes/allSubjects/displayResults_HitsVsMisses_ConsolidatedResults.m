function displayResults_HitsVsMisses_ConsolidatedResults...
...
if removeBadElectrodeData
    for iSub = 1:length(subjectIdx)
        for iRef = 1:2
            clear badElecsTMP
                badElecsTMP = union(badElecs{iRef}{subjectIdx(iSub)},declaredBadElectrodes); %#ok<*USENS>
...
for iPlot= 1:5
    ...
    plot(hPlot(iPlot,1),psdData.freqVals,psd_Att,'r'); hold(hPlot(iPlot,1),'on');
    plot(hPlot(iPlot,1),psdData.freqVals,psd_Ign,'b');
    plot(hPlot(iPlot,1),psdData.freqVals,psdBL_Att,'k');
    plot(hPlot(iPlot,1),psdData.freqVals,psdBL_Ign,'--k');
    plot(hPlot(iPlot,1),psdData.freqVals,diffPSD,'k'); 
...
for iPlot = 1:5
    for iBar = 1:length(dataIDs)
        if BaselineCondition
            attTMP = squeeze(mean(HitsAnalysisData{dataIDs(iBar)}(iPlot,:,:),3,nanFlag));
            ignTMP = squeeze(mean(MissesAnalysisData{dataIDs(iBar)}(iPlot,:,:),3,nanFlag));
            ignBLTMP = squeeze(mean(MissesAnalysisDataBL{dataIDs(iBar)}(iPlot,:,:),3,nanFlag));
...
linkaxes(hPlot2(1:5,2)); xlim(hPlot2(1,2),[0 4]); ylim(hPlot2(1,2),[-2 3])
linkaxes(hPlot3(1:5,2)); xlim(hPlot3(1,2),[0 4]); ylim(hPlot3(1,2),[-2 3])
...
for i=5:5
set(hPlot2(i,2),'yTick',[-4 0 4],'xTick',1:3,'xTickLabel',Datalabels,'XTickLabelRotation',30);
set(hPlot3(i,2),'yTick',[-4 0 4],'xTick',1:3,'xTickLabel',Datalabels,'XTickLabelRotation',30);
end
...
figName1 = fullfile(saveFolder,[protocolType '_' subString  timeEpoch '_FlickerStimuli_',showNeuralMeasure,'_' ssvepMethod, '_tapers_1' , ...
...
HitsData = cell(1,length(attendLocs)*length(ssvepFreqs)+1);
MissesData = cell(1,length(attendLocs)*length(ssvepFreqs)+1);
...
for iCount = 1:length(attendLocs)*length(ssvepFreqs)
    switch iCount
        case 1
            attLoc = 2; ign_AttLoc = 1; att_TF = 2; ign_AttTF = 3; conditionID_Hits = 3; conditionID_Misses = 9; elecNums = elecsRight;
        case 2
            attLoc = 1; ign_AttLoc = 2; att_TF = 2; ign_AttTF = 3; conditionID_Hits = 4; conditionID_Misses = 10; elecNums = elecsLeft;
        case 3
            attLoc = 2; ign_AttLoc = 1; att_TF = 3; ign_AttTF = 2; conditionID_Hits = 5; conditionID_Misses = 11; elecNums = elecsRight;
        case 4
            attLoc = 1; ign_AttLoc = 2; att_TF = 3; ign_AttTF = 2; conditionID_Hits = 6; conditionID_Misses = 12; elecNums = elecsLeft;
    end
...
HitsData(1+length(attendLocs)*length(ssvepFreqs),:,:,:) = squeeze(mean(HitsData,1,nanFlag));
MissesData(1+length(attendLocs)*length(ssvepFreqs),:,:,:) = squeeze(mean(MissesData,1,nanFlag));
...
for iDataType = 1: length(data)
    HitsDataTMP{iDataType}(1+length(attendLocs)*length(ssvepFreqs),:,:) = squeeze(mean(HitsDataTMP{iDataType},1,nanFlag));
    MissesDataTMP{iDataType}(1+length(attendLocs)*length(ssvepFreqs),:,:) = squeeze(mean(MissesDataTMP{iDataType},1,nanFlag));
end
...
for iCount = 1:length(attendLocs)*length(ssvepFreqs)
    ...
    elecs = elecNums{1};
    Hits_SSVEP_all(iCount,:,:) = squeeze(mean(dataTMP(:,subjectIdx,elecs,conditionID_Hits),1,nanFlag));
    Misses_SSVEP_all(iCount,:,:) = squeeze(mean(dataTMP(:,subjectIdx,elecs,conditionID_Misses),1,nanFlag));
end


HitsData{length(data)+1}(1,:,:) = squeeze(HitsData{1,3}(1,:,:));
HitsData{length(data)+1}(2,:,:) = squeeze(HitsData{1,3}(2,:,:));
HitsData{length(data)+1}(3,:,:) = squeeze(HitsData{1,4}(3,:,:));
HitsData{length(data)+1}(4,:,:) = squeeze(HitsData{1,4}(4,:,:));
HitsData{length(data)+1}(5,:,:) = squeeze(mean(Hits_SSVEP_all,1,nanFlag));


MissesData{length(data)+1}(1,:,:) = squeeze(MissesData{1,3}(1,:,:,:));
MissesData{length(data)+1}(2,:,:) = squeeze(MissesData{1,3}(2,:,:,:));
MissesData{length(data)+1}(3,:,:) = squeeze(MissesData{1,4}(3,:,:,:));
MissesData{length(data)+1}(4,:,:) = squeeze(MissesData{1,4}(4,:,:,:));
MissesData{length(data)+1}(5,:,:) = squeeze(mean(Misses_SSVEP_all,1,nanFlag));
