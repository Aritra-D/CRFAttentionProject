...
%     figure(c);
    for a = 1:length(aValsUnique)
        for e = 1:length(eValsUnique)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            analogData = []; pooledERPdata = []; %badTrialsAllElec = []; totTrials = 0; 
            for elecNum = [31 32 63 64];%[26+32 26 31+32 31] ;%[26+32 26 31+32 31];%
                ...
                analogData = analogDataSingleElec(goodPos,:);   
%             end
            ...
            pooledERPdata = cat(1,pooledERPdata,ERPdata);
            end
            meanPooledERPdata = mean(pooledERPdata,1);
...
%             plot(timeVals,dataToAnalyseBLCorrected,'color',[0.5 0.5 0.5],'linewidth',1); hold on;
            plot(timeVals,meanPooledERPdata,'color',contrastColor(c,:),'linewidth',1); axis tight; 
            ...
            rmsERPdata(a,e,c) = rms(meanPooledERPdata(:,erpPos)) - rms(meanPooledERPdata(:,blPos));
        end
...
plot(scaledxaxis,crf4,'o-'); hold on;
ax = gca;
